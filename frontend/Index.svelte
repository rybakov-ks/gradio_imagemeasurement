...
<script lang="ts">
	...
	$: measurement_mode = interactive && value && !streaming && active_source !== "webcam";
...
{:else}
	...
	>
		...
		<!-- В Index.svelte, в блоке interactive, в компоненте ImageUploader -->
		<ImageUploader
			...
			on:measurement={({ detail }) => gradio.dispatch("measurement", detail)}
			...
			{measurement_mode} <!-- ДОБАВЬТЕ ЭТУ СТРОКУ -->
